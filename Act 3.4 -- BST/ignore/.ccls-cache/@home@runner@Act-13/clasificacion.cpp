...
std::vector<bitacora> clasificacion::minuteBucketSort(std::vector<bitacora> array) {
	int n = array.size(), l = 0;
	std::vector<bitacora> clasificado;
	for (int i = 0; i < 60; i++) {
		int p = 0;
		while (p < n and l < n) {
			if (array[p].m == i) {
				clasificado.push_back(array[p]);
				l++;	
			}
			p++;
		}
...
void clasificacion::fullBucketSort(std::vector<bitacora>& array) {
	std::cout << "CARGANDO...\n";
...
void clasificacion::swap(std::vector<bitacora>& array, int i, int j) {
